    ...
    fn azure_login() -> Arc<DefaultAzureCredential> {
        ...
        */  
        let credential_sources = match env::var("AZURE_CLIENT_ID") {
            Ok(azure_client_id) => {
                println!("[nxfutild][az-identity] AZURE_CLIENT_ID is set, will try to use User Assigned Managed Identity");
                vec![
            ...
            Err(_) => {
                println!("[nxfutild][az-identity] AZURE_CLIENT_ID is unset, will try to use System Assigned Managed Identity");
                vec![
                    DefaultAzureCredentialEnum::ManagedIdentity(
                        ImdsManagedIdentityCredential::default()
                    ),
